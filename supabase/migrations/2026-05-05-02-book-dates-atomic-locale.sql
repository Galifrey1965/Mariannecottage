-- Migration: 2026-05-05-02-book-dates-atomic-locale.sql
-- Purpose:
--   Threads `guest_locale` through book_dates_atomic alongside `source` so the
--   /api/book caller can stamp the language the guest booked in. Defaults to
--   'en' if omitted (matches the column default).
-- Date: 2026-05-05
-- Author: Rob
...
DECLARE
  ...
  v_locale          TEXT := COALESCE(p_booking->>'guest_locale', 'en');
  v_id              UUID;
  v_reference       TEXT;
...
BEGIN
  ...
  INSERT INTO bookings (
    ...
    pending_until, cancellation_policy_id, source, guest_locale
  ) VALUES (
...
VALUES ('2026-05-05-02-book-dates-atomic-locale.sql', NOW())
