...
| **AI inbox** | Opens Gmail → drafts already written under each guest email in their language → reads for 30 seconds, hits Send (or edits). Spam and notifications classified out automatically. **Single inbox model:** all `@mariannecottage.fr` addresses (`bookings@`, `hello@`, `mark@`, `kim@`, etc.) forward to Mark's existing `mariannecottage@gmail.com` via OVH's free MX (bundled with the `.fr` registration). Gmail's "Send mail as" lets Mark reply *from* `bookings@mariannecottage.fr` over OVH's outbound SMTP — recipients see a branded domain address, Mark watches one inbox. Pre-purchase verification checklist: [`infrastructure.md` Domain section](../../infrastructure.md#domain). |
