...
|---|---|
| **Direct booking site** | Guests book at `mariannecottage.fr`, money lands in his Stripe account (cards + PayPal both flow into the same account), calendar updates automatically — no email back-and-forth required. Site carries a **prominent "Book Direct — save 5%" call-to-action** (hero banner + sticky on listing pages) alongside the BC-vs-direct price comparison (BC anchor vs `BC × 0.95` direct), to steer comparison-shoppers off OTAs. Per Mark (GitHub issue #48); a brief 10/5 refinement was reverted same day on Mark's preference. |
