...
| 11 | Unified guest inbox + AI assistant | ✅ **Mark's existing Gmail + Claude/Gemini agent** (Gmail-drafts pattern) | Detailed design in [`06a-inbox-agent-design.md`](06a-inbox-agent-design.md). Agent creates replies as Gmail drafts in the right thread — Mark uses his existing Gmail, no new UI. Three-phase trust progression starting at 100% human review. **LLM provider-agnostic** via abstraction layer: default Gemini 2.5 Flash-Lite (free tier, 1,000 requests/day), fallback to Claude via Netlify AI Gateway (no API keys to manage) for complex messages. Total cost £0–£2/month at cottage volume. ~2 day build. |
