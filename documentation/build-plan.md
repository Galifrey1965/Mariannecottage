...
|---|---|---|---|---|
| 1 | Stabilise | ~5 days | [`specs/phase-1-stabilise.md`](specs/phase-1-stabilise.md) | 🟡 code complete — pending push + Mark's domain/email infra |
...
| BC sync — stale block clearing | **S-01** | Pulled forward from Phase 2. Sync now frees `availability` rows that have dropped out of the BC feed; manually-set rows untouched. |
| BC sync — Netlify hourly scheduler | **S-02** | Pulled forward from Phase 2. Netlify Functions v2 scheduled handler at `netlify/functions/sync-bc.ts` (`@hourly`) POSTs to the sync endpoint. |
...
| Google Cloud + Business Profile delegation | infra | Mark adds Rob as Manager on the existing Google Business Profile listing (~5 min). Mark creates a `marianne-cottage` Cloud project, enables billing (no cost expected), generates a domain-restricted Maps JavaScript API key, sets it in Netlify as `PUBLIC_GOOGLE_MAPS_API_KEY`, redeploys. Adds Rob as Project Editor in IAM. Unblocks the maps swap (currently rendering placeholders) and pre-positions Hotel Center (Phase 3) and Pub/Sub (Phase 4) on the same project. Walk-through: [`setup/google-business.md`](setup/google-business.md). |
...
| ~~BC iCal scheduler~~ | ~~**S-02**~~ | ✅ moved to Phase 1 (PR 5, 2026-05-03) — landed early because the route already existed and the deliverable was self-contained backend work |
| ~~Stale-block clearing~~ | ~~**S-01**~~ | ✅ moved to Phase 1 (PR 5, 2026-05-03) |
