...
- **Status:** ✅ built 2026-07-26 (**awaiting migration application** — see below) — `retryUnnotifiedEnquiries()` in `src/lib/server/enquiry-retry.ts`, called from the existing `@daily` `/api/sweep-pending` in its own try/catch so it cannot take down the booking sweep or the retention purge. Selects `status='new' AND admin_notified_at IS NULL` (the previously-unused `enquiries_unnotified_idx`), oldest first.
  ...
  - 18 unit tests, plus a live end-to-end run against the real database on 2026-07-26 (migration applied, `EMAIL_DRY_RUN=true`, seeded `@example.invalid` fixtures since removed):
    - Two un-notified rows — one whose original send failed, one rescued from `spam` — were picked up and notified in a single run: `{"considered":2,"sent":2,"failed":0,"abandoned":0}`. Both had `notify_attempts` incremented to 1, `last_notify_attempt_at` stamped, and `notify_error` cleared. The `spam` row was correctly left alone. **The rescue → retry chain works: a false positive really does get the notification the classifier suppressed.**
    - Guest acknowledgements were suppressed exactly twice, with zero acks sent — the no-double-email guarantee holds in practice, not just in the mocks.
    - Both ceilings were then forced (one row to `notify_attempts=5`, another back-dated 40 days) and the next run returned `{"considered":0,...,"abandoned":2}` — so the give-up path and the `.or()` predicate carrying an embedded ISO timestamp both behave against real Postgres, which the mocked unit tests could not prove.
    - The run wrote its `enquiry_notify_retried` audit event as intended.
