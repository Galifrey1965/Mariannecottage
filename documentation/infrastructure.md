...
- **Notification retry:** the same `@daily` sweep re-sends the admin notice for rows still at `status='new'` with `admin_notified_at IS NULL`, capped at 5 attempts and 14 days per row and 10 sends per run (the limit is the ~10s Netlify function ceiling, not Brevo's 300/day). Retries send the admin notice only — never a second guest acknowledgement. Rows past either ceiling are counted into an `enquiry_notify_retried` audit event and left visible in the admin UI for a human.
