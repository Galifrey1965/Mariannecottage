<script lang="ts">
	...
	interface Props {
		...
		// When true, we re-fetch the booking via /api/book/[ref] until it
		// flips out of pending_payment. Disable when the parent route loaded
		// the row server-side and will invalidate on its own.
		pollClientSide?: boolean;
		// Called whenever a poll returns fresh booking data (so the wizard
		// page can keep its own copy in sync).
		onBookingUpdate?: (b: BookingShape) => void;
	}
	...
	onMount(() => {
		if (!pollClientSide) return;
		if (booking.status !== 'pending_payment') return;
		pollTimer = setInterval(async () => {
			pollAttempts += 1;
			try {
				const res = await fetch(`/api/book/${encodeURIComponent(booking.booking_reference)}`);
				if (res.ok) {
				...
				// Network blips are fine — keep polling until MAX_POLLS.
			}
			if (pollAttempts >= MAX_POLLS && pollTimer) {
				clearInterval(pollTimer);
				pollTimer = undefined;
			}
		}, 2000);
...
<div class="confirmed">
	<div class="success-header">
		{#if booking.status === 'confirmed'}
			...
			<h2 class="success-title">{t(messages, 'booking_confirm.thank_you')}</h2>
		...
		{:else if booking.status === 'pending_payment'}
			...
			<p class="success-subtitle">
				{t(messages, 'booking_confirm.confirming_subtitle')} <strong>{ref}</strong>
			</p>
		{:else}
			<div class="success-icon error">
				<span>!</span>
			</div>
			<h2 class="success-title">{t(messages, 'booking_confirm.status')}: {booking.status}</h2>
			<p class="success-subtitle">
				{t(messages, 'booking_confirm.status_other')} <strong>{ref}</strong>.
			</p>
		{/if}
	</div>
	...
	<div class="summary-card">
		<h3 class="summary-heading">{t(messages, 'booking_confirm.summary')}</h3>
...
<style>
	.confirmed { display: flex; flex-direction: column; gap: 0; }
